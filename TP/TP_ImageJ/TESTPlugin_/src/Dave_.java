...
public class Dave_ implements PlugIn {
	...
	public void run(String arg) {
		...
		ImagePlus imageSegmenteFCM = NewImage.createImage("Image segmentée par Dave",currentImage_width,currentImage_height,1,24,0);
		...
		// +1 for the noisy cluster
		double Umat[][] = new double[nbClasses+1][currentImage_nbPixels];
		// the color component
...
////////////////////////////////////////////////////////////////////
	...
	private double[][] computeMembershipDegree( double[][] distancesDataCentroids, int nbClasses,
		...
		for (int j = 0; j < currentImage_nbPixels; j++) {
			...
			// la classe bruit
			Umat[nbClasses][j] = (sum <= 1) ? 1-sum : 0;
		}
	...
	private double computePerformanceIndex(	int currentImage_nbPixels, int nbClasses, double indiceDeFlou_m, 
		...
		double squareDelta = computeSquareDelta(currentImage_nbPixels, nbClasses, distancesDataCentroids);
		...
		for (int j = 0; j < currentImage_nbPixels; j++) {
			...
			performance += squareDelta * Math.pow(Umat[nbClasses][j],indiceDeFlou_m);
		}
		
		return performance;
	}
	
	private double computeSquareDelta(int currentImage_nbPixels, int nbClasses, double[][] distancesDataCentroids) {
		double lambda = 0.2;
		double delta = 0;
		
		for (int i = 0; i < nbClasses; i++) {
			for (int j = 0; j < currentImage_nbPixels; j++) {
				delta += distancesDataCentroids[i][j];
			}
		}
		
		delta /= nbClasses * currentImage_nbPixels;
		delta *= lambda;
