...
public class PCM_ implements PlugIn {
	...
	public void run(String arg) {
		...
		ImagePlus imageSegmenteFCM = NewImage.createImage("Image segmentée par PCM",currentImage_width,currentImage_height,1,24,0);
		...
		// retrieving the 3 component (RGB) of the image
		int index = 0;
		for(int i = 0; i < currentImage_width; i++) {
			for(int j = 0; j < currentImage_height; j++) {
				...
				red[index] = (double)colorArray[0];
				green[index] = (double) colorArray[1];
				blue[index] = (double)colorArray[2];
				index++;
			}
		}
...
////////////////////////////////////////////////////////////////////
		...
		// compute the membership degree (without using the N matrix)
		...
		// calculation of n
		double n[] = this.computeN(Umat, distancesDataCentroids, indiceDeFlou_m, currentImage_nbPixels, nbClasses);
		...
		while ((iter < nbIterationMaximum) && (stab > seuilDeStabilite)) 
		{
			...
			Umat = this.computeMembershipDegree(distancesDataCentroids, n, nbClasses, currentImage_nbPixels, indiceDeFlou_m);

			// recompute n
			n = this.computeN(Umat, distancesDataCentroids, indiceDeFlou_m, currentImage_nbPixels, nbClasses);

			// Calculate difference between the previous partition and the new partition (performance index)			
			performanceIndexes[iter] = this.computePerformanceIndex(currentImage_nbPixels, nbClasses, indiceDeFlou_m, n, Umat, distancesDataCentroids);
...
////////////////////////////////////////////////////////////////////
	 ...
	 * @return Umat
	 */
	private double[][] computeMembershipDegree( double[][] distancesDataCentroids, double[] n, int nbClasses,
		...
		for (int i = 0; i < nbClasses; i++) {
			for (int j = 0; j < currentImage_nbPixels; j++) {
				Umat[i][j] = 1 / (1 + Math.pow(distancesDataCentroids[i][j]/n[i], 1/(indiceDeFlou_m-1)));
			}
		}
	...
	/**
	 * Compute the N matrix used to compute the membership degree. 
	 * 
	 * @param Umat the membership degree matrix
	 * @param distancesDataCentroids the matrix of distance between the centroids and each pixels
	 * @param indiceDeFlou_m the fuzzy index
	 ...
	 * @param nbClasses the number of classes
	 * 
	 * @return the computed N matrix
	 */
	private double[] computeN(	double[][] Umat, double[][] distancesDataCentroids, double indiceDeFlou_m,
			int currentImage_nbPixels, int nbClasses) {
		...
		for(int i=0; i<nbClasses; i++) {
			...
			for (int j = 0; j < currentImage_nbPixels; j++) {
				...
				n[i] += Math.pow(Umat[i][j],indiceDeFlou_m)*distancesDataCentroids[i][j];
			}
	...
	private double computePerformanceIndex(	int currentImage_nbPixels, int nbClasses, double indiceDeFlou_m, 
			double[] n, double[][] Umat, double[][] distancesDataCentroids) {

		double perfomanceIndex = 0;
		
		for (int i = 0; i < nbClasses; i++) {
			for (int j = 0; j < currentImage_nbPixels; j++) {
				perfomanceIndex += Math.pow(Umat[i][j],indiceDeFlou_m) * distancesDataCentroids[i][j] + n[i]* Math.pow(1 - Umat[i][j],indiceDeFlou_m);
			}
		}
